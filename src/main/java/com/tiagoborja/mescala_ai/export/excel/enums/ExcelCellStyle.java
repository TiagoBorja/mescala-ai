...
public enum ExcelCellStyle {
    THURSDAY(DayOfWeek.THURSDAY, IndexedColors.ORANGE),
    SATURDAY(DayOfWeek.SATURDAY, IndexedColors.SEA_GREEN),
    SUNDAY(DayOfWeek.SUNDAY, IndexedColors.ROSE),
    DEFAULT(null, IndexedColors.GREY_25_PERCENT);

    private final DayOfWeek day;
    private final IndexedColors color;

    ExcelCellStyle(DayOfWeek day, IndexedColors color) {
        this.day = day;
        this.color = color;
    }
    ...
    public CellStyle createWeekCellStyle(Workbook workbook) {
    ...
    public static ExcelCellStyle fromDayOfWeek(DayOfWeek day) {
        for (ExcelCellStyle style : values()) {
            if (style.day == day) {
                return style;
            }
        }
